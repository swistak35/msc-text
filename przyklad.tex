...
\item Solargraph (\cite{SOLARGRAPH}) -- dostarcza podobne funkcjonalności do tych wymienionych we wstępie, stosuje do tego analizę statyczną oraz wykorzystuje inne źródła (takie jak dokumentacja YARD \cite{YARD})
...
Jednak najtrudniejszą część, a jednocześnie najbardziej potrzebną jest skok do definicji metody. Metoda jest wywoływana na jakimś obiekcie, więc musimy znać \enquote{typ} zmiennej aby zlokalizować odpowiednią definicję.
...
Na początek warto popatrzeć na najprostsze AST, czyli literały. AST literału liczby całkowitej (np. \texttt{42}) zostanie przekształcony na pojedyńczy wierzchołek rodzaju \texttt{int} (Rys. \ref{fig:graph-int}). Rodzaj będzie potrzebny funkcji wnioskującej typy, która na podstawie rodzaju, dodatkowych parametrów wierzchołka oraz wierzchołków z krawędzi wejściowych będzie decydowała jakiego typu jest dany wierzchołek. W tym prostym przypadku wierzchołek nie ma żadnych parametrów i nie zwracamy uwagi na krawędzie wejściowe (bo ich, z metody budowania grafu, dla wierzchołka tego rodzaju nawet nie będzie) tylko przypisujemy wszystkim wierzchołkom rodzaju \texttt{int} typ \texttt{Nominal(Integer)}.
...
Ponieważ zdefiniowane stałe mają domyślnie globalną widzialność oraz mogą być przypisywane ponownie, ich obsługa nie różni się bardzo od zwykłych zmiennych globalnych. Po pierwsze, odróżnia je to, że nie są prostym identyfikatorem (jak \texttt{\$foo}), ale złożonym z kilku, niekoniecznie statycznych (jeśli \texttt{x = Foo} to \texttt{x::Bar} jest poprawną referencją do \texttt{Foo::Bar}), więc wymagają specjalnych struktur danych w bazie, algorytmu rozwiązywania tych referencji i (przez to, że referencje mogą być dynamiczne) niekoniecznie są zawsze obliczalne w statycznej analizie.
...
\begin{algorithm}
    \algsize
    \underline{function typeNodes} $(nodesWorklist, messageSendsWorklist)$:\\
    \While{!nodesWorklist.empty?}{
        \While{!nodesWorklist.empty?}{
            ...
            \For{$messageSend \in messageSendWorklist$}{
...
\begin{algorithm}
    \algsize
    \underline{function processNode} $(node)$:\\
    nodeCounter[node]++\\
    \If{$nodeCounter[node] < 100$}{
        currentType = types[node]\\
        types[node] = computeType(node, graph.incomingVertices(node))\\
        \If{$currentType \neq types[node]$}{
            \For{$targetNode \in graph.adjacentVertices(node)$}{
                nodesWorklist.push(targetNode)
            }
        }
    }
    \caption{Pseudokod funkcji typującej pojedyńczy wierzchołek}
...
Na początku sprawdzamy czy wywołanie nie pasuje do jednej z heurystyk opisanych poniżej (Alg. \ref{alg:processMessageSend}). W przeciwnym przypadku zakładamy że to jest zwykłe wywołanie, które w szczególe zostanie omówione w następnej sekcji.

\begin{algorithm}
    \algsize
    \underline{function processMessageSend} $(messageSend)$:\\
    \For{$possibleType \in types[messageSend.sendObj].possibleTypes$}{
...
\begin{algorithm}
    \algsize
    \underline{function handleConstructorSend} $(className, messageSend)$:\\
...
\begin{algorithm}
    \algsize
    \underline{function connectConstructorToCallResult} $(className, resultNode)$:\\
...
\begin{algorithm}
    \algsize
    \underline{function computeType} $(node, adjacentNodes)$:\\
    \ldots\\
...
\begin{algorithm}
    \algsize
    \underline{function handleProcCall} $(lambdaType, messageSend)$:\\
...
\begin{algorithm}
    \algsize
    \underline{function handleClassSend} $(className, messageSend)$:\\
...
\begin{algorithm}
    \algsize
    \underline{function handleInstanceSend} $(className, messageSend)$:\\
...
\begin{algorithm}
    \algsize
    \underline{function handleRegularMessageSend} $(foundMethod, messageSend)$:\\
    ...
    \For{$zsuperCall \in nodes.zsupers$}{
...
\begin{algorithm}
    \algsize
    \underline{function connectYieldsToBlock} $(yieldNodes, blockNode)$:\\
    \For{$yieldNode \in yieldNodes$}{
        lambdaIds = lambdaIdsOfBlock(blockNode)\;
        \For{$lambdaId \in lambdaIdsOfBlock(blockNode)$}{
